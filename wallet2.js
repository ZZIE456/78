...
const BNBAmount = document.querySelector("#BNBAmount");
const dogeBNBAmount = document.querySelector("#dogeBNBAmount");
...
const BNBError = document.querySelector("#BNBError");
...
const chainRPC = "https://rpc.ankr.com/BNB";
const chainName = "bnb";
// 56
//const chainIdHex = "0x5";
// 0x56
// const chainRPC = "https://bsc-dataseed.binance.org";
//const chainRPC = "https://rpc.ankr.com/BNB_goerli";
//const chainName = "bnb Goerli Testnet";
//0x38 : Binance smart chain
...
async function loadContractInfo() {
  ...
  preSaleContract = new _web3.BNB.Contract(preSaleABI, PRE_SALE_ADDRESS);
  usdtContract = new _web3.BNB.Contract(ercABI, USDT_ADDRESS);
  const _dogeContract = new _web3.BNB.Contract(ercABI, DOGE_ADDRESS);
  
  const [tokenPrice, tokenSold, tokenBalance, raised] = await Promise.all([
    preSaleContract.mBNBods.tokenPrice().call(), 
    preSaleContract.mBNBods.tokensSold().call(), 
    _dogeContract.mBNBods.balanceOf(PRE_SALE_ADDRESS).call(), 
    preSaleContract.mBNBods.getRaised().call()
  ])
  
  const tokenPriceValue = _web3.utils.fromWei(tokenPrice, "BNBer");
  const tokenSoldValue = _web3.utils.fromWei(tokenSold, "BNBer");
  const tokenBalanceValue = _web3.utils.fromWei(tokenBalance, "BNBer");
  const raisedValue = _web3.utils.fromWei(raised['0'], "BNBer");
  const targetValue = _web3.utils.fromWei(raised['1'], "BNBer");;
...
function loadContractInfo() {
  ...
  usdtContract = new _web3.BNB.Contract(ercABI, USDT_ADDRESS);
  const _dogeContract = new _web3.BNB.Contract(ercABI, DOGE_ADDRESS);
  preSaleContract.mBNBods.tokenPrice().call().then(v =>{
    const value = _web3.utils.fromWei(v, "BNBer");
    [].forEach.call(document.querySelectorAll(".preSalePriceUSD"), function (item) {
      item.innerHTML = `$${value}`;
    });
  });

  preSaleContract.mBNBods.tokensSold().call().then(v =>{
    const value = _web3.utils.fromWei(v, "BNBer");
    document.querySelector("#totalSold").innerHTML = `${numberWithCommas(value)}`;
  });

  _dogeContract.mBNBods.balanceOf(PRE_SALE_ADDRESS).call().then(v =>{
    const value = _web3.utils.fromWei(v, "BNBer");
    document.querySelector("#totalSupply").innerHTML = `${numberWithCommas(value)}`;
  });

  preSaleContract.mBNBods.getRaised().call().then(v =>{
    const raisedValue = _web3.utils.fromWei(v['0'], "BNBer");
    const targetValue = _web3.utils.fromWei(v['1'], "BNBer");;
    document.querySelector("#raisedValue").innerHTML = `$${numberWithCommas(raisedValue)}`;
    document.querySelector("#targetValue").innerHTML = `$${numberWithCommas(targetValue)}`;
  });
}
...
function fetchContribution() {
  if (preSaleContract) {
    preSaleContract.mBNBods.contributions(selectedAccount).call().then(v =>{
      const value = _web3.utils.fromWei(v, "BNBer");
...
async function fetchAccountData() {
  ...
  if (chainId != currentChainId) {
    if (provider.bridge != "https://bridge.walletconnect.org") {
      ...
      // https://bnb.stackexchange.com/a/62217/620
      const alert = document.querySelector("#alert-error-https");
      alert.style.display = "block";
      document
        .querySelector("#btn-connect")
  ...
  const accounts = await web3.BNB.getAccounts();
...
async function refreshAccountData() {
  ...
  // with bnb node via JSON-RPC and loads chain data
  // over an API call.
  document.querySelector("#btn-connect").setAttribute("disabled", "disabled");
  await fetchAccountData(provider);
  document.querySelector("#btn-connect").removeAttribute("disabled");

  // preSaleContract = new web3.BNB.Contract(preSaleABI, PRE_SALE_ADDRESS);
  // await reloadInfo();
}
...
function getTokenPreSaleAmount(paymentToken, paymentAmount, target) {
  const paymentAddress = paymentToken === "BNB" ? BNB_ADDRESS : USDT_ADDRESS;
  const paymentAmountWei = _web3.utils.toWei(paymentAmount)
  preSaleContract.mBNBods.getTokenPresaleAmount(paymentAddress, paymentAmountWei).call().then(v =>{
    const value = _web3.utils.fromWei(v, "BNBer");
    console.log("felix getTokenPreSaleAmount", value)
	if(value<20000){
		showError(BNBError, "Please purchase at least 20000 Dogo");
...
function getTokenPreSalePrice(paymentToken, buyingAmount, target) {
  ...
  preSaleContract.mBNBods.getTokenPresalePrice(paymentAddress, buyingAmountWei).call().then(v => {
    const value = _web3.utils.fromWei(v, "BNBer");
...
async function buy() {
  ...
  switch (lastHitInputName) {
    case 'BNBAmount':
	  ...
	  value = _web3.utils.toWei(BNBAmount.value);
      console.log("felix going to buy with BNB", value);

      accountBalance = await _web3.BNB.getBalance(selectedAccount);

      if (new BN(accountBalance).lt(new BN(value))) {
        showError(BNBError, "Insufficient BNB balance, please check your account balance");
        hideProcessing();
        return;
      }

      data = preSaleContract.mBNBods.buy(BNB_ADDRESS, value).encodeABI();

      provider
        .request({
          mBNBod: 'BNB_sendTransaction',
          params: [
            {
        ...
        .then((txHash) => {
          ...
          const interval = setInterval(function() {
            _web3.BNB.getTransactionReceipt(txHash, function(err, rec) {
              if (rec) {
    ...
    case 'dogeBNBAmount':
      hideError(BNBError);
      showProcessing();
      dogeValue = _web3.utils.toWei(dogeBNBAmount.value);
      console.log("felix going to buy with BNB at", dogeValue);
	  
      data = preSaleContract.mBNBods.buyExactTokens(BNB_ADDRESS, dogeValue).encodeABI();
      
      value = await preSaleContract.mBNBods.getTokenPresalePrice(BNB_ADDRESS, dogeValue).call();
    ...
    bnValue = new BN(value).div(powerTwelve);
      ...
      usdtBalance = await usdtContract.mBNBods.balanceOf(selectedAccount).call();
      ...
      usdtContract.mBNBods.allowance(selectedAccount, PRE_SALE_ADDRESS).call().then(allowance => {
        console.log("felix allowance", allowance);

        if (bnValue.gt(new BN(allowance))) {
          console.log("felix doesn't have enough allowance");
          data = usdtContract.mBNBods.approve(PRE_SALE_ADDRESS, bnString).encodeABI();
          provider
            .request({
              mBNBod: 'BNB_sendTransaction',
              params: [
              ...
              const interval = setInterval(function() {
                ...
                _web3.BNB.getTransactionReceipt(txHash, function(err, rec) {
                  if (rec) {
                    ...
                    data = preSaleContract.mBNBods.buy(USDT_ADDRESS, value).encodeABI();
                    provider
                      .request({
                        mBNBod: 'BNB_sendTransaction',
                        params: [
                      ...
                      .catch((error) => {
                        console.error("felix buy error", error);
                        showError(BNBError, "Insufficient USDT balance, please check your account balance");
                        hideProcessing();
                      });
                  }
                });
    ...
    case 'dogeUsdtAmount':
      ...
      value = await preSaleContract.mBNBods.getTokenPresalePrice(USDT_ADDRESS, dogeValue).call();
      ...
      allowance = await usdtContract.mBNBods.allowance(selectedAccount, PRE_SALE_ADDRESS).call();
      console.log("felix allowance", allowance);
            ...
            const interval = setInterval(function() {
              ...
              _web3.BNB.getTransactionReceipt(txHash, function(err, rec) {
                if (rec) {
                  ...
                  clearInterval(interval);
                  console.log("felix has enough allowance");
                  data = preSaleContract.mBNBods.buyExactTokens(USDT_ADDRESS, dogeValue).encodeABI();
                  provider
                    .request({
                      mBNBod: 'BNB_sendTransaction',
                      params: [
...
window.addEventListener("load", async () => {
  ...
  BNBAmount.addEventListener('keyup', (event) => {
    dogeBNBAmount.disabled = true;
    lastHitInputName = "BNBAmount";
    debounce(() => getTokenPreSaleAmount("BNB", event.target.value, dogeBNBAmount))();
  });
  dogeBNBAmount.addEventListener('keyup', (event) => {
    BNBAmount.disabled = true;
    lastHitInputName = "dogeBNBAmount";
    debounce(() => getTokenPreSalePrice("BNB", event.target.value, BNBAmount))();
  });
